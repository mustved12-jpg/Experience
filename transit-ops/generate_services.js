...
const mockDataContent = `// mockData.js
...
const storageContent = \`export const delay = (ms = 300) => new Promise(resolve => setTimeout(resolve, ms));
...
const makeServiceContent = (name, initialDataKey, prefix) => \`import { mockData } from '../data/mockData';
import { delay, getStorageData, setStorageData } from '../utils/storage';

const KEY = 'transitOps_\${name}';

export const \${name}Service = {
  getAll: async () => {
    await delay();
    return getStorageData(KEY, mockData.\${initialDataKey});
  },
  getById: async (id) => {
    await delay();
    const items = getStorageData(KEY, mockData.\${initialDataKey});
  ...
  update: async (id, updates) => {
    await delay();
    let items = getStorageData(KEY, mockData.\${initialDataKey});
...
const authServiceContent = \`import { delay, getStorageData, setStorageData } from '../utils/storage';

const KEY = 'transitOps_auth_user';
...
fs.writeFileSync(path.join(root, 'data', 'mockData.js'), mockDataContent);
fs.writeFileSync(path.join(root, 'utils', 'storage.js'), storageContent);
fs.writeFileSync(path.join(root, 'services', 'authService.js'), authServiceContent);
fs.writeFileSync(path.join(root, 'services', 'vehicleService.js'), makeServiceContent('vehicle', 'vehicles', 'v'));
fs.writeFileSync(path.join(root, 'services', 'driverService.js'), makeServiceContent('driver', 'drivers', 'd'));
fs.writeFileSync(path.join(root, 'services', 'tripService.js'), makeServiceContent('trip', 'trips', 't'));
fs.writeFileSync(path.join(root, 'services', 'maintenanceService.js'), makeServiceContent('maintenance', 'maintenance', 'm'));
fs.writeFileSync(path.join(root, 'services', 'fuelService.js'), makeServiceContent('fuel', 'fuel', 'f'));
fs.writeFileSync(path.join(root, 'services', 'expenseService.js'), makeServiceContent('expense', 'expenses', 'e'));

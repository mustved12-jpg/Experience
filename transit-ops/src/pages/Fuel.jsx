...
export const Fuel = () => {
  ...
  const [formData, setFormData] = useState({
    vehicleId: '', fuelLiters: 0, fuelCost: 0, date: ''
  });
  ...
  const columns = [
    ...
    { header: 'Liters', render: (row) => `${row.fuelLiters} L` },
    { header: 'Cost', render: (row) => `$${row.fuelCost.toLocaleString()}` },
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="p-6">
        <div className="mb-6">
          <SearchBar value={search} onChange={setSearch} placeholder="Search by vehicle ID..." />
        </div>
        {loading ? (
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Log Fuel Fill-up">
        <form onSubmit={handleSubmit} className="space-y-4 pt-2">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input required type="number" step="0.1" className="mt-1 block w-full rounded-md border-slate-300 shadow-sm focus:border-brand-500 focus:ring-brand-500 border p-2" value={formData.fuelLiters} onChange={e => setFormData({...formData, fuelLiters: Number(e.target.value)})} />
            </div>
            <div>
              <label className="block text-sm font-medium text-slate-700">Cost ($)</label>
              <input required type="number" step="0.01" className="mt-1 block w-full rounded-md border-slate-300 shadow-sm focus:border-brand-500 focus:ring-brand-500 border p-2" value={formData.fuelCost} onChange={e => setFormData({...formData, fuelCost: Number(e.target.value)})} />
            </div>
          </div>
          <div>
